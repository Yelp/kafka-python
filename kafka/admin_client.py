...
class AdminClient(object):
    ...
    def _send_request(self, request, max_retry):
        controller_id = self._send_controller_request()
        if self.client.ready(controller_id) == False:
            for _ in range(0, max_retry): 
                if self.client.ready(controller_id) == True:
    ...
    def create_topics(
        ...
        return self._send_request(request, max_retry) 

    def delete_topics(self, topics, timeout, max_retry=10):
